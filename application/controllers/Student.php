...
class Student extends CI_Controller { 
    ...
    function __construct() {
     ...
     function dashboard() {
        error_log('DEBUG: Entrando a Student::dashboard()');
        if ($this->session->userdata('student_login') != 1) {
            error_log('DEBUG: No session student_login');
            redirect(base_url(), 'refresh');
        }
        $page_data['page_name'] = 'dashboard';
        $page_data['page_title'] = get_phrase('student Dashboard');
        error_log('DEBUG: Antes de cargar vista backend/index');
        $this->load->view('backend/index', $page_data);
        error_log('DEBUG: Vista backend/index cargada');
    }
	/******************* / student dashboard code to redirect to student page if successfull login** */
